...
Future<List<ListItem>> getItems() async {
  ...
  try {
    final response = await http.get(
      Uri.parse(
          'http://192.168.1.8:5000/shopping-list/get-users-shopping-list-mobile'),
      headers: {
...
Future<void> bought(int itemID) async {
  ...
  try {
    final response = await http.put(
      Uri.parse('http://192.168.1.8:5000/storage/add-item-from-sList-mobile'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      },
...
Future<void> delete(int itemID) async {
  ...
  try {
    final response = await http.delete(
      Uri.parse('http://192.168.1.8:5000/shopping-list/delete-item-mobile'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      },
...
Future<void> uploadItem(String amount, String ingredient) async {
  ...
  try {
    final response = await http.post(
      Uri.parse(
          'http://192.168.1.8:5000/shopping-list/add-a-shopping-list-item'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      },
...
Future<void> updateItem(String amount, String ingredient, int id) async {
  ...
  try {
    final response = await http.put(
      Uri.parse(
          'http://192.168.1.8:5000/shopping-list/update-a-shopping-list-mobile'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      },
